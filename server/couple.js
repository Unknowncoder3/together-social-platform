...
io.on('connection',socket=>{
 socket.on('couple:join',()=>{
  ...
  socket.data.coupleRelationshipId=rel.id;const peers=[...io.sockets.adapter.rooms.get(room)||[]].map(id=>io.sockets.sockets.get(id)?.user?.id).filter(Boolean);socket.join(room);socket.emit('couple:joined',{relationshipId:rel.id,roomId:store.rooms.find(r=>r.relationshipId===rel.id)?.id||null,peers,occupancy:Math.min(2,new Set(peers).size+1)});socket.to(room).emit('couple:peer-joined',{userId:socket.user.id});
 });
